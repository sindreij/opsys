...
public class Simulator implements Constants
{
	...
	public Simulator(Queue memoryQueue, Queue cpuQueue, Queue ioQueue, long memorySize,
		...
		cpu = new Cpu(cpuQueue, gui, statistics);
		io = new IO(ioQueue, gui, statistics);
		clock = 0;
		// Add code as needed
    }
	...
	private void switchProcess() {
		Process p = cpu.stopProcess(clock);
		if (p != null) {
			if (p.finished()) {
		...
		p = cpu.startProcess(clock);
		if (p != null) {
			long timeToIo = p.getTimeToNextIoOperation();
			if (timeToIo < maxCpuTime) {
	...
	private void processIoRequest() {
		Process p = cpu.stopProcess(clock);
		assert p != null;
		assert p.getTimeToNextIoOperation() == 0;
	...
	private void endIoOperation() {
		Process p = io.processFinished(clock);
		assert p != null;
		cpu.insertProcess(p, clock);
	...
	private void startIoOperation() {
		Process p = io.startProcess(clock);
		if (p != null) {
			long ioTime = 1 + (long)(2*Math.random()*avgIoTime);
			eventQueue.insertEvent(new Event(END_IO, clock+ioTime));
		}
