...
public class BarbershopGui extends JFrame implements Constants, Gui, ChangeListener {
	...
	public BarbershopGui(String title) {
		...
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				int barber_customers = 0;
				for(int i = 0; i < NOF_BARBERS; i++) {
					System.out.printf("Barber #%d had %d customers\n", i, barbers[i].getCustomers());
					barber_customers += barbers[i].getCustomers();
				}
	...
	public void startSimulation() {
		...
		cashier = new Cashier();
		for(int i = 0; i < NOF_BARBERS; i++) {
			barbers[i] = new Barber(queue, this, i, cashier);
	...
	private Image loadImage(Toolkit tk, String file, MediaTracker tracker) {
		...
		Image result = tk.createImage(url);
